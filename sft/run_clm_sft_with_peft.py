...
class TextGenerationCallback(transformers.TrainerCallback):
    def __init__(self):
        self.input_texts = [ 
            "در مورد کشور ایران چه میدانی؟",
            "یک شعر بنویس",
            "حالت خوبه؟",
            "ما بعد از مرگ به کجا میرویم؟",
            "به من یک فیلم معرفی کن",
            "با کلمات بهار و مدرسه یک جمله بنویس",
            "What do you know about the country of Iran?",
            "Write a poem.",
            "How are you?",
            "Where do we go after death?",
            "Recommend a movie to me.",
            "Write a sentence with the words 'spring' and 'school'."
        ] 

    def on_evaluate(self, args, state, control, model, tokenizer, **kwargs):
        for input_text in self.input_texts:
            prompt_input = generate_prompt(instruction=input_text)
            inputs = tokenizer(prompt_input, return_tensors="pt")
            inputs.to("cuda")
            
            with torch.no_grad():
                with torch.autocast("cuda"): 
                    generated_output = model.generate(**inputs, max_length=200,temperature=0.7,top_p=1,do_sample=True)
                    generated_text = tokenizer.decode(generated_output[0], skip_special_tokens=True)
            gen_text = generated_text.replace(prompt_input,"")
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            train_dataset = build_instruction_dataset(
                ...
                data_cache_dir = os.path.join(data_args.data_cache_dir, "-".join(files)+"_text"),
